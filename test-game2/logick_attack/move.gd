...
const ChessUtils = preload("res://path/to/ChessUtils.gd")
...
func calculate_move(piece_type: PieceType, direction: Direction, steps: int, rows: int, columns: int, current_position: Vector2i) -> int:
	var movement = _piece_movements[piece_type].get(direction, Vector2i.ZERO)
	if movement == Vector2i.ZERO and direction != Direction.ZERO:
		push_error("Invalid direction for this piece type")
		return current_index

	var new_position = ChessUtils.calculate_new_position(current_position, movement, steps)
	var new_index = ChessUtils.clamp_position(new_position, rows, columns)
	if new_index == -1:
		return current_index  # Ошибка при проверке границ
	return new_index

func handle_input(piece_type: PieceType, direction: Direction, steps: int, rows: int, columns: int, current_position: Vector2i) -> int:
