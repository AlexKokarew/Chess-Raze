...
const ChessUtils = preload("res://logick_attack/ChessUtilities.gd")
const ConflictQueue = preload("res://class_trash/ConflictQueue.gd")

func calculate_attack_position(current_position: Vector2i, attack_vector: Vector2i, steps: int, rows: int, columns: int) -> int:
	var attack_position = ChessUtils.calculate_new_position(current_position, attack_vector, steps)
	var attack_index = ChessUtils.clamp_position(attack_position, rows, columns)
	if attack_index == -1:
		push_error("Attack move out of bounds!")
	return attack_index
